...
class AuctionSimulator:
    def __init__(self, seller_prices, buyer_prices, alpha_factors, epsilon, auction_type):
        ...
        self.bid_increase_factor = np.random.rand(
    ...
    def run_levelled_commitment_auction(self):
        ...
        for round_number in range(self.number_of_rounds):
            ...
            self.commited_buyer_bids = np.zeros(
                (self.number_of_buyers, self.number_of_auctions), dtype=bool)

            # Buyer and Seller Profits for the Round
            buyer_profits = np.zeros(
                (self.number_of_buyers, self.number_of_auctions))
            seller_profits = np.zeros(self.number_of_auctions)

            # Buyer profits including annuled auctions
            total_buyer_profits = np.zeros(
                (self.number_of_buyers, self.number_of_auctions))

            # Buyer prices for the round adapted with alpha factors
            self.buyer_prices[:, :, round_number] = self.alpha_factors * \
                self.seller_prices[:, round_number]

            for k in range(self.number_of_auctions):
                buyer_indices = [i for i in range(self.buyer_prices.shape[0])]

                self.auction_results["Round {}".format(
                    round_number)]["Auction {}".format(k)] = {}

                logging.info('Sk value for current auction: {}'.format(
                    self.seller_prices[k, round_number]))
                logging.info('Alpha Values for current auction: {}'.format(
                    self.alpha_factors[:, k]))
                buyer_prices_for_auction = self.buyer_prices[:,
                                                             k, round_number]
                logging.info('Buyer Prices befor Annuling: {}'.format(
                    buyer_prices_for_auction))
                ...
                for buyer_index, x in previous_auctions_won:
                    buyer_prices_for_auction[buyer_index] = self.buyer_prices[buyer_index, k, round_number] - \
                        buyer_profits[buyer_index, x] - self.epsilon * \
                        self.buyer_prices[buyer_index, x, round_number]
                    if buyer_prices_for_auction[buyer_index] < self.seller_prices[k, round_number]:
                        if buyer_index in buyer_indices:
                            buyer_indices.remove(buyer_index)

                buyer_prices_for_auction = self.buyer_prices[buyer_indices,
                                                             k, round_number]

                logging.info("Alpha Factors for the buyers: {}".format(
                    self.alpha_factors))
                logging.info('Buyer Prices for the current auction: {}'.format(
                    buyer_prices_for_auction))

                # Calculate Market Price
                market_price_for_auction = self.calculate_market_price(
                    buyer_prices_for_auction)

                # Append to list to display later
                self.market_price_developments.append(
                    [market_price_for_auction, round_number * self.number_of_auctions + k])

                # Determine Auction Winner
                auction_winner_index = self.get_auction_winner(
                    market_price_for_auction, buyer_prices_for_auction)
                auction_winner = buyer_indices[auction_winner_index]
                logging.info(
                    "Auction Winner is Buyer: {}".format(auction_winner))
                ...
                winning_bid = self.get_winning_bid(
                    buyer_prices_for_auction, auction_winner_index)
                buyer_profits[auction_winner,
                              k] = market_price_for_auction - winning_bid

                logging.info(
                    'Buyer Profits for Auction: ( May get annuled later): {}'.format(buyer_profits))
                # Update seller profits for this auction
                seller_profits[k] = winning_bid
                total_buyer_profits[auction_winner,
                                    k] = buyer_profits[auction_winner, k]
                ...
                for buyer_index, x in previous_auctions_won:
                    if buyer_index == auction_winner:
                        ...
                        if buyer_profits[auction_winner, k] > buyer_profits[auction_winner, x]:
                            ...
                            total_buyer_profits[auction_winner, k] = buyer_profits[auction_winner,
                                                                                   k] - self.epsilon * self.buyer_prices[buyer_index, x, round_number]

                            # Seller profits for the annuled round is the penalty.
                            seller_profits[x] = self.epsilon * \
                                self.buyer_prices[buyer_index, x, round_number]
            ...
            for x, auction_winner in np.argwhere(self.commited_buyer_bids.T):
                ...
                self.adapt_alpha_factors(market_price_for_auction, buyer_prices_for_auction, [
                                         i for i in range(self.buyer_prices.shape[0])], auction_winner, x)

                logging.info("Buyer Profits: {}".format(self.buyer_profits))
                logging.info("Seller Profits: {}".format(self.seller_profits))

                self.auction_results["Round {}".format(
                    round_number)]["Auction {}".format(x)]["Winner"] = auction_winner
                self.auction_results["Round {}".format(round_number)]["Auction {}".format(
                    x)]["Market Price"] = market_price_for_auction
                self.auction_results["Round {}".format(round_number)]["Auction {}".format(
                    x)]["Buyer Profits"] = self.buyer_profits
                self.auction_results["Round {}".format(round_number)]["Auction {}".format(
                    x)]["Seller Profits"] = self.seller_profits
    ...
    def run_pure_auction(self):
        ...
        for round_number in range(self.number_of_rounds):
            ...
            for k in range(self.number_of_auctions):
                ...
                # Get seller profit
                self.calculate_seller_profits(
                    k, buyer_prices_for_auction, auction_winner_index)
                # Get buyer profit
                self.calculate_buyer_profits(
                    auction_winner, market_price_for_auction, buyer_prices_for_auction, auction_winner_index)
                ...
                self.auction_results["Round {}".format(
                    round_number)]["Auction {}".format(k)]["Winner"] = auction_winner
                self.auction_results["Round {}".format(round_number)]["Auction {}".format(
                    k)]["Market Price"] = market_price_for_auction
                self.auction_results["Round {}".format(round_number)]["Auction {}".format(
                    k)]["Buyer Profits"] = self.buyer_profits
                self.auction_results["Round {}".format(round_number)]["Auction {}".format(
                    k)]["Seller Profits"] = self.seller_profits

                self.adapt_alpha_factors(
                    market_price_for_auction, buyer_prices_for_auction, buyer_indices, auction_winner, k)
    ...
    def adapt_alpha_factors(self, market_price_for_auction, buyer_prices_for_auction, buyer_indices, auction_winner, k):
        buyers_above_market_price = [buyer_indices[i] for i in np.where(
            buyer_prices_for_auction >= market_price_for_auction)[0]]
        ...
        self.alpha_factors[buyers_increase_bid, k] = self.bid_increase_factor[buyers_increase_bid] * \
            self.alpha_factors[buyers_increase_bid, k]
    ...
    def calculate_market_price(self, prices):
        ...
        logging.info(
            "Market Price for current auction: {}".format(market_price))
        return market_price
...
if __name__ == "__main__":
    ...
    for sellers in range(auction_simulator.number_of_auctions):
        for rounds in range(auction_simulator.number_of_rounds):
            ...
            price_variance.append(
                [np.var(auction_simulator.buyer_prices[:, sellers, rounds]), rounds, sellers])
    ...
    print("-"*50)
    print("-"*50)
    print("Final Seller Profits:")
    print(auction_simulator.seller_profits)
    print("-"*50)
    print("Mean Seller Profits:")
    print(s_profits)
    print("-"*50)
    print("Final Buyer Profits:")
    print(auction_simulator.buyer_profits)
    print("-"*50)
    print("Mean Buyer Profits:")
    print(b_profits)
    plt.show()
